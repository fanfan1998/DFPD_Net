...
from dfpd_net.lfd import LFDModule
import torch
import numpy as np
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.block_wise_pool = nn.AvgPool2d(kernel_size=4, stride=4, padding=0)
        self.layer1 = self._make_layer(block, 64, layers[0])
        self.LFD1 = LFDModule(256)
        self.bn_ce1 = nn.BatchNorm2d(256)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.LFD2 = LFDModule(512)
        self.bn_ce2 = nn.BatchNorm2d(512)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.LFD3 = LFDModule(1024)
        self.bn_ce3 = nn.BatchNorm2d(1024)
...
def _resnet(arch, inplanes, planes, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        print(f"    • {len(unexpected_keys)} unexpected keys (should be 0)")
