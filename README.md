...
3. Specify the `dataset_root_path` in the config.yaml file. Search for the line that mentions dataset_root_path. By default, it looks like this: ``dataset_root_path: ./datasets``.
...
The training script supports both configuration file and command-line arguments. You can specify training/testing datasets and model name via command-line arguments, which will override the settings in `config.yaml`.

**Option 1: Using command-line arguments (recommended for distributed training)**
...
**Option 2: Using the provided training script**

You can also use the provided `train_ddp.sh` script for convenient multi-GPU training:

  ```
  bash train_ddp.sh xception
  ```

The script supports two model options: `resnet50` or `xception` (default: `xception`).

**Option 3: Using configuration file only**

If you have configured all parameters in `config.yaml`, you can run:

  ```
  python train.py
  ```

**Note**: The evaluation is automatically performed during training. The model will be evaluated on the test datasets specified in the configuration or command-line arguments after each epoch.